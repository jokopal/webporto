# MENLIMAN JOYFAL GULO

**GIS & Remote Sensing  |  Geospatial Data Science & Web GIS  |  Environmental, Agricultural & Coastal Mapping**

+62 823-9703-8718  |  menlimanjoyfalgulo@gmail.com  |  Yogyakarta, Indonesia  
[linkedin.com/in/menlimanjoyfalgulo](https://www.linkedin.com/in/menlimanjoyfalgulo)  |  [jokopal.github.io/webporto](https://jokopal.github.io/webporto/)  |  [github.com/jokopal](https://github.com/jokopal)  
_Fresh graduate (19 June 2026)  |  Open to work  |  Willing to relocate and to on-site / roster placement anywhere in Indonesia_

> Master resume / profile database. Last updated on graduation, 19 June 2026. Single source of truth for tailoring role-specific CVs.

## Professional Summary

Cartography and Remote Sensing (GIS) graduate from Universitas Gadjah Mada (GPA 3.60/4.00, KIP scholar), graduated June 2026, and an independent GIS developer. I turn satellite imagery and field data into decision-ready insight across environmental, coastal, agricultural and mining applications, using ArcGIS and QGIS, remote sensing (Sentinel-2, LiDAR, radar, UAV), machine learning (Random Forest, SVM, U-Net, YOLOv8), Python and Google Earth Engine, and I ship results as interactive web-GIS dashboards and open-source tools. Experienced across research, industry and international conservation, with proven leadership, field readiness and strong communication. Open to work and ready for on-site or roster placement anywhere in Indonesia.

## Core Skills

- **GIS and Remote Sensing:** ArcGIS Pro, ArcGIS Enterprise, ArcGIS Dashboards, QGIS, Google Earth Engine, ENVI, SNAP, Agisoft, FUSION, DJI Terra, Surfer, GeoDa, CloudCompare, CPCe; land-cover, thematic, coastal and carbon-stock mapping.
- **Remote Sensing Data:** Sentinel-2, Landsat 8/9, PlanetScope, WorldView, LiDAR, radar/SAR, UAV/drone aerial imagery; UAV mapping pilot.
- **Machine Learning and Geo-AI:** Random Forest, SVM, U-Net, YOLOv8, CNN, PCA/KPCA, feature engineering, accuracy assessment; predictive modelling for agricultural yield and water quality.
- **Programming and Databases:** Python (NumPy, Pandas, scikit-learn, rasterio, FastAPI, OpenCV, TensorFlow), SQL, PostgreSQL/PostGIS, R, SQLite, Supabase, Git/GitHub.
- **Web GIS and Development:** OpenLayers, React + Vite, Electron, JavaScript/TypeScript, TailwindCSS, AlpineJS, Google Apps Script, Role-Based Access Control (RBAC).
- **Field and Survey Instruments:** GNSS/GPS, robotic total station, theodolite, waterpass, TLS, sonar, underwater camera; outdoor SOP and field safety.
- **Domain Knowledge:** blue carbon, benthic/seagrass/mangrove mapping, water-quality remote sensing (TSS, Chlorophyll-a, salinity), mine reclamation and revegetation (Kepmen ESDM 344/2025), AMDAL and RKL-RPL, permit compliance, hazard mitigation, ESG and sustainability.
- **Professional:** leadership, project management, public speaking, scientific writing, coaching and training, teamwork, adaptability, fast learning.

## Professional Experience

### Assistant Specialist, Remote Sensing and GIS (COAST TA Project)  |  Jul 2026 - Present
*Seacrest Indonesia | Contract, Remote (Central Java coast)*

- Delivered a full satellite dataset, land-classification and stratification maps, RS methodology documentation, multi-layer overlays and village-level recap: all six analytical deliverables on schedule under a results-based contract for a blue-carbon rehabilitation feasibility study.

### Remote Sensing Analyst, Coastal Water-Quality  |  Jun 2026 - Present
*Wildlife Conservation Society | Freelance (West Nusa Tenggara & South Bolaang Mongondow)*

- Monitored water quality across 347 aquaculture zones and 16 Marine Protected Areas with bi-seasonal Sentinel-2 (TSS, Chlorophyll-a, salinity); benchmarked retrieval algorithms against in-situ data and confirmed compliance with PP 22/2021.
- Led multi-temporal TSS monitoring (ZOM 497) with 17 Sentinel-2 scenes and 7 algorithms; delivered technical reports for marine spatial planning and conservation zoning.

### Spatial Compliance Analyst, License Department  |  Sep 2025 - Dec 2025
*PT Berau Coal | Internship, On-site (Berau, East Kalimantan)*

- Monitored mining permit and concession boundaries, land overlaps and violations with GIS and remote monitoring for regulatory compliance and risk mitigation.
- Built and shipped a full-stack Web GIS licensing dashboard (OpenLayers, Supabase/PostGIS, RBAC) with automated Apps Script, WhatsApp and email notifications.

### Student Trainee, Talent Development Program (Geospatial & Data Analytics)  |  Aug 2025 - Dec 2025
*Sinar Mas Agribusiness and Food (TDC UGM x Sinarmas) | Hybrid*

- Delivered an AI-driven ESG capstone for the palm-oil business ("Palm Oil, but Make It Cool", Group 3, 25 Nov 2025); strengthened data analytics, project management and strategic communication with cross-functional teams.

### Co-Trainer, Intermediate Google Earth Engine Training  |  Dec 2025
*GGGI Indonesia (Jakarta, On-site)*

- Facilitated GEE live-coding (Sentinel-2, Random Forest, accuracy assessment) supporting standardised national benthic-habitat mapping and blue-carbon monitoring.

### Volunteer Geospatial Analyst, Disaster Charter Rapid Flood Mapping  |  Dec 2025
*Universitas Gadjah Mada (Aceh, West & North Sumatra)*

- Pre and post-disaster image interpretation and digitisation of flood and landslide extents and affected infrastructure, attributed to BRIN geospatial standards for impact assessment and emergency response.

### Surveyor, Tsunami Early-Warning System  |  Aug 2025
*CV Spiro Energy | Freelance, Field (Central Java)*

- Inspected and tested EWS siren/transmitter towers with the BPBD team and applied radio-frequency programming via Motorola CPS for reliable early-warning communication.

### Research Assistant, Blue Carbon Research Group (+ Laboratory & Teaching Assistant)  |  Aug 2024 - Present
*Faculty of Geography, Universitas Gadjah Mada | Hybrid*

- Applied machine learning (Random Forest, SVM), Google Earth Engine and Sentinel-2 to benthic-habitat and blue-carbon mapping; feature engineering (deglint, DII, PCA/KPCA) and accuracy assessment.
- Field surveyor for the National Seagrass Map (UGM, KKP, BRIN, BIG, University of Queensland): 16 benthic spot-check sites (Madura), 91 transect samples (Pulau Banyak, Aceh Singkil), snorkelling photo-transects at Sanur (Bali), processing across 7 Indonesian regions.
- Laboratory and teaching assistant for 54 students across spatial data infrastructure and enterprise GIS (ArcGIS Enterprise, GeoServer, PostGIS, GeoNode), active remote sensing (LiDAR, radar; SNAP, ENVI, FUSION), and land surveying.

### Speaker, Scientific Writing Training  |  Oct 2024
*Building Information Modelling (BIM) Community, DTS SV UGM*

- Delivered comprehensive scientific-writing training (academic structure, effective data presentation, reference management) to the BIM community at the Department of Civil Engineering, Vocational School, UGM.

### Operational Coordinator, Mine-Eye Aerial Acquisition (Astranauts Finalist)  |  Oct 2024 - Jan 2025
*Mine Eye Gama, Astranauts & Innovlab 2024 by Astra Digital*

- Coordinated cross-functional teams to build Mine-Eye, a remote LiDAR/drone geospatial-processing solution for PT Pama Persada Nusantara mining areas; prototyped a Ground Control Station and a drone-LiDAR tracking web app.

### Undergraduate Researcher, Blue Carbon & Seagrass Mapping (ML + Multispectral RS)  |  Dec 2024
*UGM, Fundamental Research Grant 2024*

- Applied multispectral remote sensing and machine learning to rapid mapping of seagrass carbon stocks (feature engineering, regression-model comparison, spatial upscaling); outputs published in an international conference and a journal.

### Team Lead, Urban Portable Agriculture (SOTECH Pertamina 2024)  |  May 2024 - Jul 2024
*Mina Padi Program, PT Kilang Pertamina RU III Plaju x AntaraDjaya (Plaju, Palembang)*

- Led a team to build an IoT integrated vertical-farming system (98.7% accuracy, sub-5-second response); secured 1st Runner-Up in the technology category.

### Main Author & Presenter, 7th National Geomatics Seminar  |  May 2024
*Geospatial Information Agency (BIG), Cibinong*

- Presented and published a predictive-modelling study on air-concentration change from deforestation in the IKN area; selected for the official national proceedings.

### Surveyor, Street-Lighting (PJU) Data Collection Phase V  |  Oct 2023 - Nov 2023
*UPT PJU, Dinas PUPKP Kota Yogyakarta*

- Surveyed and verified kWh-meter data for about 400 street-lighting points; supported data analysis for lighting-infrastructure optimisation.

### Freelance Video Editor  |  Nov 2023 - Jan 2024
*ExcellencIA Learning Center and Consultant | Remote*

- Edited video courses and consulting content (DaVinci Resolve, CapCut), improving audio-visual quality and visual storytelling for an education platform.

## Projects and Products (Independent)

- **BlueMap: Coastal AutoMapper (v2.0):** open-source (MIT, Zenodo DOI) offline desktop app for satellite habitat mapping with ML (Random Forest, SVM), sunglint and water-column correction, and accuracy assessment; built solo in Python (FastAPI, rasterio, scikit-learn) with React and Electron. 3,016 launch impressions.
- **Palm DeepLearning Model (GeoAI):** YOLOv8/U-Net/CNN pipeline for oil-palm detection (2,264 trees, F1 0.79), planting-distance and blank-spot analytics, crown-size estimation and tree-health diagnosis; Python (OpenCV, TensorFlow). 8,342 impressions. github.com/jokopal/Palm_DeepLearningModel
- **WEB GIS Spatial-Licence Dashboard (PT Berau Coal):** full-stack mining-licence dashboard (OpenLayers, Supabase/PostGIS, RBAC) for real-time compliance monitoring; most-viewed LinkedIn post (4,029 impressions).
- **Shp.Aid:** AI-assisted web GIS toolkit for shapefile and geospatial-data workflows (shpaid.com).
- **L.E.A.D Logbook Automation (PT Berau Coal):** React.js/TailwindCSS frontend for a digital logbook and report-automation app (team of 3).
- **Recommendation System (Web App):** React + TypeScript + Supabase data-management and recommendation web application with a custom, brand-styled UI.

## Leadership and Organizations

- **President, Geoscience Community UGM (2024):** led a 60-plus member organization across research, education and competitions; secured 6 national competition wins and 1 proceedings publication; managed budget, logistics and stakeholders.
- **General Coordinator, UKK CUP 2024:** led an inter-campus event across 11 universities (4 arts and 5 sports competitions); managed cross-university teams, budget, logistics and venue.
- **Head of Division 4 (Logistics & PR), MAPALA GEGAMA (2023-2024):** coordinated procurement, resources and digital communications; survival, navigation and field SOP, including the rock-climbing division.
- **Head of Internal Affairs, Indonesian Climate Change Initiative UGM (2022-2024):** directed internal communications and cross-department collaboration for sustainability programs.
- **Co-Facilitator, PPSMB Pionir UGM (2023):** delivered 21 orientation modules to 42 new students; public speaking, mentoring and assessment.
- **Additional involvement:** BEM Faculty of Geography (Social Affairs), KMK & UKK UGM (Media), Gamasubmission (Public Relations), INTERSECT (mentor), Sonar HMSaiG and campus committees.

## Education

### Universitas Gadjah Mada, Yogyakarta  |  Jul 2022 - Jun 2026
*Bachelor of Science, Cartography and Remote Sensing (GIS) - graduated June 2026*

- GPA 3.60/4.00; Kartu Indonesia Pintar (KIP) scholarship awardee.
- Relevant coursework: Geo-AI, Active Remote Sensing, Spatial Analysis and Modelling, Spatial Programming, Terrain and Imagery Analysis, Analytical and Thematic Cartography, Geospatial Statistical Methods, Survey and Mapping Project Management.

### SMK Negeri 1 Lolofitu Moi, Nias Barat  |  2019 - 2022
*Computer and Network Engineering (Teknik Komputer dan Jaringan)*

- Final grade 92.00/100; foundation in computer maintenance and wireless networking.

## Publications

- Mapping Seagrass Dynamics in a Developing Coastal Area: A Case Study of Kuta Mandalika, Lombok Island, Indonesia (Regional Studies in Marine Science).
- Comparison of PCA, KPCA, and Regression Models for Seagrass Percent Cover Mapping Using Sentinel-2 (SPIE Proceedings, Asia-Pacific Remote Sensing Conference 2024, Kaohsiung, Taiwan).
- Air Concentration Changes Due to Deforestation of the IKN Area (7th National Geomatics Seminar Proceedings, BIG, 2024).
- IoT-Based Urban Portable Agriculture System: Technical Validation of a Low-Cost Hydroponic Monitoring Platform (lead author, 2024).
- Flood Vulnerability Analysis of the Bengawan Solo Watershed using the Overlay Method, Sukoharjo, Central Java (2024).
- Fintech Innovation: Enhancing the Sustainability of Micro, Small and Medium Enterprises (2025).
- BlueMap: Coastal AutoMapper - open-source software release (MIT, Zenodo DOI).

## Honors and Awards

- Grant Recipient, PKM-GFT (Student Creativity Program, Futuristic Ideas), Ministry of Education, Culture, Research and Technology.
- Gold Medal and 2nd Place, Diponegoro Science Competition, 2024 (national).
- 1st Runner-Up, Technology Category, SOTECH Pertamina Competition, 2024 (national).
- 2nd Place, Business Challenge Track, Astranauts by PT Astra Digital, 2024 (national).
- 3rd Place, Scientific Writing Competition, LOGIN Mercator Geodesy UGM, 2024.
- National Poetry Competition Finalist and author of the book "No Longer Looking for Cemara".

## Certifications and Training

- Thematic Google Earth Engine Training: Deforestation and Air Pollution (Geoacces, 2024).
- Basic Geotechnical Investigation using Resistivity and 2D Plaxis (CV Geotama Multi Resources, 2023).
- Tsunami Inundation Modeling and Evacuation Route Design (PT Langen Teknik Satria, 2023).
- Kelompok Materi Pelatihan Dasar (KMPD); KMPL Pekerja Kantor dan Dapur.

## Languages

Bahasa Indonesia (native)  |  Bahasa Nias (native)  |  English (professional working: international publication and conference presentation, University of Queensland collaboration).

## Personal Details (reference)

- **Date of birth:** 2 January 2005
- **Place of birth:** Hilimanaze, Nias, North Sumatra, Indonesia
- **Nationality:** Indonesian
- **Gender:** Male
- **UGM student ID (NIM):** 22/492934/GE/09790
- **Academic email:** menlimanjoyfalgulo@mail.ugm.ac.id
